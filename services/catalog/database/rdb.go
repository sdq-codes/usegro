...
func InitRedisClient(redisConfigs []config.Redis) error {
	...
	if len(addrs) == 1 {
		SingleRdb = redis.NewClient(&redis.Options{
			Addr:     addrs[0],
	...
	_, err := SingleRdb.Ping(context.Background()).Result()
	if err != nil {
		return err
	}
...
func AddQueuePrefix(key string) string {
	if queuePrefix == "" {
		m.Lock()
		defer m.Unlock()
		queuePrefix = config.GetConfig().App.NameSlug
	}
