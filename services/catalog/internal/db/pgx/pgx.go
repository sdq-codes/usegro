...
func InitPgConnectionPool(postgresConfig config.Postgres) error {
	...
	connStr := fmt.Sprintf("host=%s port=%d authentication=%s password=%s dbname=%s sslmode=disable search_path=%s",
		postgresConfig.Host,
		postgresConfig.Port,
...
func GetPgxConn() *pgxpool.Conn {
	...
	//
	//return conn
	panic("GetPgxConn called")
}

func InitSchema(ctx context.Context, postgresConfig config.Postgres, schema string) (err error) {
	connStr := fmt.Sprintf("host=%s port=%d authentication=%s password=%s dbname=%s sslmode=disable",
		postgresConfig.Host,
		postgresConfig.Port,
		postgresConfig.Username,
		postgresConfig.Password,
	...
	// Ignore error if schema already exists or if the authentication doesn't have permission to create schema
	pgConn.Exec(
		ctx,
