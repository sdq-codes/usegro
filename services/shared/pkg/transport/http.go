...
func MakeHTTPRequest(ctx context.Context, req HttpRequest) (*http.Response, error) {
	...
	for {
		...
		sharedlogger.Log.Debug(fmt.Sprintf("making HTTP request to %s headers: %v", req.Url, req.Headers))
		sharedlogger.Log.Debug(fmt.Sprintf("request body to %s is %s", req.Url, buf))
		...
		if res.StatusCode == http.StatusUnauthorized && req.onRenewBearer != nil {
			sharedlogger.Log.Debug("got 401 response, token renewer function is provided, renewing a token..")
		...
		sharedlogger.Log.Debug(fmt.Sprintf("HTTP response status from %s is %s", req.Url, res.Status))
		return res, nil
	}
}

// RequestAndParseJSONBody calls MakeHTTPRequest then reads the response body
// and unmarshals it to the result pointer.
func RequestAndParseJSONBody(ctx context.Context, req HttpRequest, result interface{}) error {
	...
	sharedlogger.Log.Debug(fmt.Sprintf("response body from %s is %s", req.Url, body))
