...
func TestGetUsers(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:               "test get all authentication",
			body:               "",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			resp := e.GET("/api/v1/authentication").Expect()
...
func TestGetUserByID(t *testing.T) {
	tests := []struct {
		...
		{
			name:               "test get authentication by id",
			expectedStatusCode: http.StatusOK,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			resp := e.GET("/api/v1/authentication/1").Expect()
...
func TestCreateUser(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:               "test create authentication success",
			body:               newAccount,
		...
		{
			name:               "test create authentication duplicate email",
			body:               newAccount,
			expectedStatusCode: http.StatusUnprocessableEntity,
			expectedSchema:     readJSONToString(t, "json_response_schema/error_422.json"),
			expectedCode:       422,
			expectedMessage:    "authentication email already taken",
		},
		// err 400 bad request
		{
		...
		{
			name: "test create authentication field validation required",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			resp := e.POST("/api/v1/authentication").WithJSON(tt.body).Expect()
