...
func Pull(ctx context.Context, key string) (string, error) {
	...
	_, delErr := database.GetRedisClient().Del(ctx, key).Result()
	if delErr != nil {
...
func SetForever(ctx context.Context, key string, value interface{}) error {
	key = database.AddPrefix(key)
	err := database.GetRedisClient().Set(ctx, key, value, 0).Err()
	if err != nil {
...
func Remove(ctx context.Context, key string) error {
	key = database.AddPrefix(key)
	_, err := database.GetRedisClient().Del(ctx, key).Result()
	if err != nil {
...
func Increment(ctx context.Context, key string, increment int64) (int64, error) {
	key = database.AddPrefix(key)
	val, err := database.GetRedisClient().IncrBy(ctx, key, increment).Result()
	if err != nil {
...
func Decrement(ctx context.Context, key string, decrement int64) (int64, error) {
	key = database.AddPrefix(key)
	val, err := database.GetRedisClient().DecrBy(ctx, key, decrement).Result()
	if err != nil {
