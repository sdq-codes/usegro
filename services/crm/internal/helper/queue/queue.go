...
import (
	...
	"github.com/usegro/services/crm/internal/apps/base/repositories"
	"github.com/usegro/services/crm/internal/job"
...
func NewQueue(key string, jobRepositoryInterface repositories.JobRepositoryInterface) *Queue {
	return &Queue{
		Key:              database.AddQueuePrefix(key),
...
func (q *Queue) Enqueue(ctx context.Context, tx *gorm.DB, jobs ...*models.Job) error {
	rdbClient := database.GetRedisClient()
	...
	for _, j := range jobs {
		...
		_, err = q.repo.AddJob(ctx, tx, &models.Job{
			ID:          j.ID,
...
func (q *Queue) Dequeue(ctx context.Context, timeout time.Duration) (*job.Job, error) {
	...
	if err := q.repo.UpdateJobStatus(ctx, j.ID, job.StatusProcessing); err != nil {
...
func (q *Queue) RemoveProcessed(ctx context.Context, jobID uuid.UUID, jobError error) error {
	...
	for _, queueItem := range queues.Val() {
		...
		if j.ID == jobID {
			...
			if err := q.repo.UpdateJobStatus(ctx, j.ID, job.StatusCompleted); err != nil {
				logger.Log.Error("Error updating job status in postgres", zap.Error(err))
				return err
			}
...
// If the job failed, add it to the failed_jobs list
func handleFailedJob(ctx context.Context, rdbClient redis.Cmdable, queue string, repo repositories.JobRepositoryInterface, j job.Job, sourceKey string, failedJobsKey string) error {
	if j.MaxAttempts == 0 || j.Attempts < j.MaxAttempts {
		...
		if err := repo.UpdateJobStatus(ctx, j.ID, job.StatusPending); err != nil {
			logger.Log.Error("Error updating job status in postgres", zap.Error(err))
			return err
		}
		...
		// update job status in postgres
		if err := repo.UpdateJobStatus(ctx, j.ID, job.StatusFailed); err != nil {
			logger.Log.Error("Error updating job status in postgres", zap.Error(err))
			return err
		}

		// Add failed job to postgres
		_, err := repo.AddFailedJob(ctx, models.FailedJob{
			JobID:    j.ID,
			Queue:    queue,
			Payload:  j.Payload,
...
func (q *Queue) RetryFailedByJobID(ctx context.Context, jobID uuid.UUID) error {
	...
	// update job status in postgres
	if err := q.repo.UpdateJobStatus(ctx, jobID, job.StatusPending); err != nil {
		logger.Log.Error("Error updating job status in postgres", zap.Error(err))
		return err
	}

	// remove job from failed_jobs in postgres
	if err := q.repo.RemoveFailedJob(ctx, jobID); err != nil {
...
func (q *Queue) RetryAllFailed(ctx context.Context) (int, error) {
	...
	for {
		...
		// update job status in postgres
		if err := q.repo.UpdateJobStatus(ctx, j.ID, job.StatusPending); err != nil {
			logger.Log.Error("Error updating job status in postgres", zap.Error(err))
			return count, err
		}

		// remove job from failed_jobs in postgres
		if err := q.repo.RemoveFailedJob(ctx, j.ID); err != nil {
			logger.Log.Error("Error removing job from failed_jobs in postgres", zap.Error(err))
			return count, err
		}
